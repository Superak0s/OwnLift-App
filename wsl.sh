...
WSL_PROJECT="$HOME/OwnLift-App"

# ─── Parse flags ───────────────────────────────────────────────────────────────
# --prebuild : run a full `expo prebuild --clean` instead of an incremental one.
#              Use this after adding/upgrading native deps. No longer asked
#              interactively during the build.
DO_CLEAN=false
for arg in "$@"; do
    case "$arg" in
        --prebuild) DO_CLEAN=true ;;
    esac
done

echo "=== OwnLift App Release Script (WSL Build) ==="
echo ""
sleep 1
...
# Prebuild mode is now controlled by the --prebuild flag (see top of script),
# not an interactive prompt.
if [ "$DO_CLEAN" = true ]; then
    echo "Running full clean prebuild (--prebuild flag set)..."
...
# ─── [5/6] Build the APK via local-expo-build ────────────────────────────────
...
# --no-prebuild : we already ran expo prebuild + gradle.properties tuning above
# --no-bump     : version bump already handled in [1/6]
# --no-sync     : skip pushing versionCode to EAS (we don't use EAS)
#
# One-time setup required before this works (not part of the script):
#   cd "$WSL_PROJECT" && npx local-expo-build keystore import /path/to/your.jks
echo ""
echo "[5/6] Building release APK via local-expo-build..."
cd "$WSL_PROJECT"

npx local-expo-build build android --apk --no-sync --no-bump --no-prebuild
