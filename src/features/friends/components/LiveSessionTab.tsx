...
const makeLiveDotSt = (colors: any) =>
  StyleSheet.create({
    dot: {
      width: 8,
      height: 8,
...
const makeBblStyles = (colors: any) =>
  StyleSheet.create({
    bubble: {
      width: 70,
      height: 70,
...
const makeExStStyles = (colors: any) =>
  StyleSheet.create({
    card: {
...
export default function LiveSessionTab({
  ...
  > | null => {
    ...
    const exercises: ProgramExercise[] = Array.isArray(day.exercises)
      ? day.exercises
      : day.split
        ? Object.values(day.split).flatMap((pw) => pw?.exercises ?? [])
        : day.people
