...
export function VersionGuard({
  ...
  const validateServerUrl = (url: string): { valid: boolean; message?: string } => {
    ...
    if (
      ...
      return { valid: false, message: "HTTP is not secure. Please use HTTPS for production servers." }
    try {
      new URL(trimmed)
      return { valid: true }
    } catch {
  ...
  const handleSaveServerUrl = async () => {
    ...
    if (!validation.valid) {
      alert("Invalid URL", validation.message || "Please enter a valid URL", [{ text: "OK" }], "error")
      return
    }
    ...
    if (isLocal) {
      alert(
        ...
        [
          ...
          { text: "Continue", onPress: async () => { await saveUrl(url) } },
        ],
        "warning",
      )
      return
    ...
    return (
      <View style={styles.container}>
        <View style={styles.loadingBox}>
          ...
          <Text style={styles.loadingText}>Verifying server compatibility...</Text>

          {/* Allow changing server even during the loading state */}
          <TouchableOpacity style={styles.serverBadge} onPress={handleOpenServerModal}>
            <Text style={styles.serverIcon}>🌐</Text>
            <View style={styles.serverBadgeContent}>
              <Text style={styles.serverBadgeLabel}>Server</Text>
              <Text style={styles.serverBadgeUrl} numberOfLines={1}>{currentServerUrl}</Text>
            </View>
        ...
        <View style={styles.errorBox}>
          ...
          <View style={styles.errorContent}>
            ...
            <View style={styles.versionInfo}>
              ...
              <Text style={styles.versionValue}>{versionStatus.clientVersion ?? "Unknown"}</Text>
              <Text style={styles.versionLabel}>Server Version:</Text>
              <Text style={styles.versionValue}>{versionStatus.serverVersion ?? "Unable to connect"}</Text>
            </View>
          </View>
          ...
          <View style={styles.buttonContainer}>
            <TouchableOpacity
              style={[
                styles.button,
                versionStatus.clientNeedsUpdate ? styles.buttonSecondary : styles.buttonPrimary,
              ]}
            ...
            {versionStatus.clientNeedsUpdate && (
              <TouchableOpacity
                ...
                onPress={() =>
                  void Linking.openURL("https://github.com/Superak0s/SuperGym-App/releases")
                }
              >
  ...
  return (
    <>
      <Text style={styles.modalDescription}>
        Enter the URL of your workout tracker server (including http:// or https://)
      </Text>
      <TextInput
      ...
      <View style={styles.modalWarning}>
        ...
        <Text style={styles.modalWarningText}>
          Always use HTTPS for production servers to ensure your data is encrypted
        </Text>
      </View>
...
const makeStyles = (colors: any) =>
  StyleSheet.create({
    ...
    loadingBox: { alignItems: "center", justifyContent: "center", paddingHorizontal: 24, width: "100%" },
    loadingEmoji: { fontSize: 48, marginBottom: 12 },
    loadingText: { fontSize: 16, color: colors.textSecondary, textAlign: "center", marginBottom: 24 },
    errorBox: {
    ...
    errorTitle: { fontSize: 20, fontWeight: "700", color: colors.textPrimary, textAlign: "center" },
    ...
    serverBadgeLabel: { fontSize: 12, color: colors.textMuted, fontWeight: "600", marginBottom: 2 },
    serverBadgeUrl: { fontSize: 13, color: colors.textPrimary, fontWeight: "500" },
    ...
    footnote: { fontSize: 12, color: colors.textMuted, textAlign: "center", fontStyle: "italic" },
    // Modal
    modalDescription: {
      fontSize: 15,
      color: colors.textSecondary,
    ...
    modalWarningText: { flex: 1, fontSize: 13, color: colors.textSecondary, lineHeight: 18 },
    resetButton: {
    ...
    modalHelperText: { fontSize: 13, color: colors.accent, textAlign: "center", fontStyle: "italic" },
  })